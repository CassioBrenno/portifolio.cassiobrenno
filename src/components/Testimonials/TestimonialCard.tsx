    ...
    >
      ...
      <div className="flex items-center justify-center gap-1.5">
        {Array.from({ length: 5 }, (_, idx) => (
          <StarIcon
            key={idx}
            className={`text-xl ${idx < stars ? 'text-tag fill-current' : 'text-neutral/30'}`}
          />
        ))}
      </div>
      <div className="mx-auto h-[68px] w-[68px] overflow-hidden rounded-full border-2 border-neutral/30">
        <Image
          src={image}
          alt={name}
          width={68}
